...
execute if score #hit abch.explosive_sight.raycast matches 0 positioned ~-0.05 ~-0.05 ~-0.05 as @e[ type=#abchc:living_base, tag=!abch.explosive_sight.ray, tag=!global.ignore, dx=0, sort=nearest ] run function abchc:modifiers/explosive_sight/raycast/check_hit_entity
