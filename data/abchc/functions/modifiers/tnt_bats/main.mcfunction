...
execute as @e[ type=tnt, tag=abch.tnt_bats.tnt, tag=!abch.tnt_bats.detonated ] at @s unless entity @e[type=bat,tag=abch.tnt_bats,distance=..1.5] run function abchc:modifiers/tnt_bats/detonate

#> Spawn new bats from non-bat entities
execute as @e[type=#aestd1:mobs,tag=!abch.tnt_bats.spawned] at @s unless entity @s[type=bat] unless entity @a[distance=..10] unless score tnt_bats.spawn abch.config matches 0 run function abchc:modifiers/tnt_bats/try_spawn
