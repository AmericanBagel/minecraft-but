...
execute if score no_gravity.travel minecraft_but.config matches 1 as @a[scores={minecraft_but.no_gravity.sneak=1..}] at @s if block ~ ~-.1 ~ #minecraft_but.core:air run tp @s ~ ~-.1 ~

# Raise players who look up
execute if score no_gravity.travel minecraft_but.config matches 1 as @a[x_rotation=-90..-30] at @s if block ~ ~.1 ~ #minecraft_but.core:air run tp @s ~ ~.1 ~