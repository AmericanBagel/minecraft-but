#> minecraft_but.coreraft_but.core:modifiers/keep_moving/check
#
# Get player's position, compare it with previous position, and run either movement or still functions
#
# @context player
# @within minecraft_but.coreraft_but.core:modifiers/keep_moving/
...
execute as @a[ tag=!minecraft_but.blacklist, tag=!global.ignore, tag=!minecraft_but.keep_moving.blackliminecraft_but.corerun function minecraft_but.core:modifiers/keep_moving/current_pos

# Compare pos scores with a bias towards moving
execute unless score @s minecraft_but.keep_moving.x = $p minecraft_but.keep_moviminecraft_but.corerun function minecraft_but.core:modifiers/keep_moving/move
execute unless score @s minecraft_but.keep_moving.y = $p minecraft_but.keep_moviminecraft_but.corerun function minecraft_but.core:modifiers/keep_moving/move
execute unless score @s minecraft_but.keep_moving.z = $p minecraft_but.keep_moviminecraft_but.corerun function minecraft_but.core:modifiers/keep_moving/move

execute if score @s minecraft_but.keep_moving.x = $p minecraft_but.keep_moving.x if score @s minecraft_but.keep_moving.y = $p minecraft_but.keep_moving.y if score @s minecraft_but.keep_mminecraft_but.core.z = $p minecraft_but.keep_moving.z run function minecraft_but.core:modifiers/keep_moving/still
...
execute as @a[ tag=!minecraft_but.blacklist, tag=!global.ignore, tag=!minecraft_but.keep_moving.blacklist ] aminecraft_but.corerun function minecraft_but.core:modifiers/keep_moving/later_pos