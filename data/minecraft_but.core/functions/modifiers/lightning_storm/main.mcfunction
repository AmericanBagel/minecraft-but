#> minecraft_but.coreraft_but.core:modifiers/lightning_storm/main
...
execute as @a at @s if score @s minecraft_but.lightning >= $interval minecraft_but.lightminecraft_but.corerun function minecraft_but.core:modifiers/lightning_storm/spawner

# After one second, spawn lightning and kill lightning marker
execute as @e[ type=marker, name="minecraft_but.lightning", tag=!minecraft_but.blacklist, tag=!minecraft_but.lightning_storm.blacklist ] at @s if score @s minecraft_but.minecraft_but.corening matches 20.. run function minecraft_but.core:modifiers/lightning_storm/lightning
