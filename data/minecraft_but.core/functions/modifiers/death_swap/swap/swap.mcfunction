#> minecraft_but.coreraft_but.core:modifiers/death_swap/swap/swap
# Swap @s to @r
...
execute if score $players minecraft_but.death_swap matches 2.. as @r[tag=!minecraft_but.death_swap.swapped,tag=!minecraft_but.death_swap.blacklist,tag=!minecraft_but.blacklist,tagminecraft_but.corebal.ignore] at @s run function minecraft_but.core:modifiers/death_swap/swap/swap

# If there's only one player left, tp player to already swapped player
execute if score $players minecraft_but.death_swap matches 1 as @a[tag=!minecraft_but.death_swap.swapped,tag=!minecraft_but.death_swap.blacklist,tag=!minecraft_but.blacklist,tagminecraft_but.corebal.ignore] at @s run function minecraft_but.core:modifiers/death_swap/swap/last_swap