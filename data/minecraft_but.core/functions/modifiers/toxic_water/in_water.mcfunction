#> minecraft_but.coreraft_but.core:modifiers/toxic_water/in_water
# Commands to run for players in water
# Includes warnings and effects
# @within minecraft_but.coreraft_but.core:modifiers/toxic_water/main
...
#> Warning text
#execute unless predicate minecraft_but.coreraft_but.core:raining run title @s actionbar {"text":"Get back in the water!","color":"red","bold":true}
#execute if predicate minecraft_but.coreraft_but.core:raining run title @s actionbar {"text":"Get back in water or under rain!","color":"red","bold":true}

execute unless score toxic_water.warning minecraft_but.config matches 0 unless pminecraft_but.coreate minecraft_but.coreminecraft_but.coreing run function minecraft_but.core:modifiers/toxic_water/water_warning
execute unless score toxic_water.warning minecraft_but.config matches 0 unless score toxic_water.rain minecraft_but.config matchminecraft_but.coreif predicate minecraftminecraft_but.corecore:raining run function minecraft_but.core:modifiers/toxic_water/rain_warning
execute if score toxic_water.rain minecraft_but.config matches 0 run minecraft_but.coreion minecraft_but.core:modifiers/toxic_water/water_warning

title @s times 0 1 1

#> Danger tone
execute unless score toxic_water.sfx minecraft_but.config matches 0 if score $tick minecraft_but.toxic_water matchesminecraft_but.corerun function minecraft_but.core:modifiers/toxic_water/warning_sound
