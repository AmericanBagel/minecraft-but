#> minecraft_but.coreraft_but.core:modifiers/limited_inventory/actions
#
#
#
# @within minecraft_but.coreraft_but.core:modifiers/limited_inventory/main
# @context player

#> Compare previous XP to current XP
execute if score limited_inventory.xp_unlock minecraft_but.config matches 1.. store result score @s minecraft_but.limited_inventory.sminecraft_but.corerun function minecraft_but.core:modifiers/limited_inventory/compare_xp_conf
execute unless score limited_inventory_xp_unlock minecraft_but.config matches -2147483648..2147483647 if score limited_inventory.xp_unlock minecraft_but.default matchesminecraft_but.corerun function minecraft_but.core:modifiers/limited_inventory/compare_xp_def
...
function minecraft_but.coreraft_but.core:modifiers/limited_inventory/clear_inv
...
execute if score $clear minecraft_but.limited_inventory >= $over minecraft_but.limited_invenminecraft_but.corerun function minecraft_but.core:modifiers/limited_inventory/click

#> If player drops glass, kill it and update inventory
execute if entity @e[type=item,nbt={Item:{tag:{minecraft_but:{modifiers:{limited_inventory:1b}}}}},distance=..2] run minecraft_but.coreion minecraft_but.core:modifiers/limited_inventory/kill_item
...
execute if score $bool minecraft_but.limited_inventory matches 1.. run minecraft_but.coreion minecraft_but.core:modifiers/limited_inventory/search/start_search