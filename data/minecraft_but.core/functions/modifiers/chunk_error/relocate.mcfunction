scoreboard players add @s minecraft_but.chunk_error.limit 1
execute if score @s minecraft_but.chunk_error.limit matches 5.. run kill @s
...
execute as @e[ type=armor_stand, name="minecraft_but.chunkError" ] at @s positioned ~ 256 ~ run function minecraft_but.core:modifiers/chunk_error/recursion
...
execute as @e[ type=armor_stand, name="minecraft_but.chunkError" ] at @s run function minecraft_but.core:modifiers/chunk_error/error_sound
...
execute as @e[ type=armor_stand, name="minecraft_but.chunkError", tag=minecraft_but.CEvoid ] at @s if score @s minecraft_but.chunk_error.total = @p minecraft_but.chunk_error.total run function minecraft_but.core:modifiers/chunk_error/relocate