...
const namespaceDir = (__dirname, '../data/abchc/functions/');
const menuDir = path.join(__dirname, '../data/abchc/functions/menu/');
const coreDir = path.join(__dirname, '../data/abchc/functions/menu/modifiers/core/');
        ...
        */
        ...
        and an array of different strings for string type. */
            ...
            e.g. abchc:menu/modifiers/core/anvil_rain/rate/reset.mcfunction */
                ...
                e.g. abchc:menu/modifiers/core/anvil_rain/rate/up.mcfunction */
					fs.writeFileSync(
                        ...
                        function abchc:menu/modifiers/core/${modifier.id}/${
							cf.id
						}/update_string
                ...
                e.g. abchc:menu/modifiers/core/anvil_rain/rate/down.mcfunction */
					fs.writeFileSync(
						...
						outdent`
                        #> abchc:menu/modifiers/core/${modifier.id}/${
							cf.id
						}/down
                        # Increase ${modifier.id} config score ${cf.id} by 1
                        # Generated with the script at '../src/index.js'
            ...
            `;
                ...
                e.g. abchc:menu/modifiers/core/anvil_rain/rate/toggle.mcfunction */
					fs.writeFileSync(
...
for (let i = 0; i < modifiers.length; i++) {
	enableModifiersFunction += `execute if score $out random matches ${
		i + 1
	} run scoreboard players set ${modifiers[i].id} abch.random.toggle 1\n`;
}
fs.writeFileSync(
